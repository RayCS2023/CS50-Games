...
function PlayState:render()
    ...
    --render key power up
    if self.player.keyPowered > 0 then
        love.graphics.draw(gTextures['locks-and-keys'], gFrames['locks-and-keys'][self.player.keyPowered], 0,  VIR_HEIGHT - TILE_SIZE)
    end
end
...
function PlayState:updateCamera()
    ...
    self.camX = math.max(0,
        math.min(TILE_SIZE * self.tileMap.width - VIR_WIDTH,
        self.player.x - (VIR_WIDTH / 2 - 8)))
